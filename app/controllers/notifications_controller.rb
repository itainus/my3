class NotificationsController < WebsocketRails::BaseController
  ...
  def on_user_notifications_bind
    Rails.logger.info "[DEBUG INFO] ############## NotificationsController - on_user_notifications_bind ##########"
  end

  def self.notify_user user_id, msg
    Rails.logger.info "[DEBUG INFO] ############## notify_user - user_id = #{user_id} - msg = #{msg.as_json} ##########"
    connection = WebsocketRails.users[user_id]
    Rails.logger.info connection
    if true
      connection.send_message :notifications, msg, :namespace => :user
    else
